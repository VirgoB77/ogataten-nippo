...
まとめる鍵は番地まで見る。町丁目でまとめ直したら店の数が76減った（2026-09-19）。
**ただし内訳は、別の店に吸われたのが16で、残り60は `addr_key_town` が
空だったために落ちただけ**（共通仕様4節③）。
最初は76を全部「別の店に吸われた」と読んで、そう書いて出した。分けて数えたら
5分の1だった。**ひとつの数字の動きを、ひとつの原因のせいにしない。**
...
def main():
    with open(ALL, encoding="utf-8") as f:
        recs = json.load(f)
    rows = build(recs)
    doc = {"generated_at": runday.today(), "count": len(rows), "stores": rows}
