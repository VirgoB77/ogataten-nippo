#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""共通仕様 3.1 と 3.2 を、人の判断ではなくコードで守るための層。

  https://github.com/VirgoB77/ogataten-nippo/blob/main/docs/kyotsu-shiyo.md

出力する値は必ずここを通す。通さない経路を作らない。
姉妹サイト（開発系・競売統計・犯罪統計）と同じ内容にすること。
直したらリンク先の5節も直す。

なぜ要るか
  大店立地法の届出者は、ほとんどが法人だが、個人が23件まざっていた。
  設置者名と地番の両方をそのまま出していた。行政の縦覧は4か月で消えるが、
  こちらのサイトは消えない。「消えない」ことが、そのまま責任になる。
"""

import json
import os
import re
import unicodedata

# ---------------------------------------------------------------- 法人かどうか

# 法人格を表す語。これを1つも含まなければ個人として扱う（安全な側に倒す）。
# 丸囲み文字（㈱㈲）と括弧書き（(株)）は、公報と自治体の一覧表で実際に多い。
# 共通仕様5節の最低限の一覧に、実物で出てきたものを足してある。
CORP_WORDS = (
    "株式会社", "有限会社", "合同会社", "合資会社", "合名会社", "相互会社",
    "特定目的会社", "投資法人", "有限責任事業組合",
    "一般社団法人", "公益社団法人", "一般財団法人", "公益財団法人",
    "社団法人", "財団法人", "医療法人", "学校法人", "宗教法人", "社会福祉法人",
    "独立行政法人", "地方独立行政法人", "国立大学法人", "特定非営利活動法人",
    "弁護士法人", "税理士法人",
    "生活協同組合", "農業協同組合", "漁業協同組合", "事業協同組合", "協同組合", "組合",
    "公社", "公団", "事業団", "機構", "振興会", "協会", "連合会", "商工会",
    "会館", "センター", "COOP", "コープ", "生協",
    # 括弧書き。全角・半角が混ざっているので _norm() でそろえてから見る
    "(株)", "(有)", "(同)", "(資)", "(名)", "(福)", "(医)", "(相)",
    # 丸囲み
    "㈱", "㈲", "㈳", "㈶", "㈴", "㈻", "㈷",
    # 「株赤ちゃん本舗」のように㈱が潰れた表記。人名に「株」は出てこない。
    # 「有」は有田・有村など姓に出るので、単独では入れない
    "株",
    # 外国法人
    "Co.", "Ltd", "Inc", "LLC", "L.L.C", "Corp", "K.K.", "PLC",
    "S.L", "S.A", "N.V", "B.V", "GmbH", "A/S", "Pty",
    "エルエルシー", "リミテッド", "コーポレーション", "ホールディングス",
)

# 国・地方公共団体。法人格の語を持たないが個人ではない。
#
# **「◯村」で終わるだけでは、地方公共団体と決めない。**
#
# 2026-09-19 の監査（r1-39）で出た。前はこう書いていた——
#
#     |(都|道|府|県|市|区|町|村)$
#
# **「中村」「木村」「田村」「西村」「大村」「北村」「野村」「今村」。**
# 日本でいちばん多い姓が8つとも法人扱いになり、**氏名と地番がそのまま出た。**
#
# 直し方は**実在の名前と突き合わせる**こと。総務省の団体コードの一覧が
# `data/ref/jis-codes.json` に在る（`ref_jis.py` が取ってくる）。
#
# **一覧が手に入らないときは「地方公共団体ではない」とみなす。**
# そちらに倒すと伏せる側に寄る。**その既定値で動いたとき外に出るのは
# 氏名と地番**なので、止まる側に倒す（9節）。
PUBLIC_RE = re.compile(
    r"^(国|日本国)$"
    r"|(都|道|府|県|市|区|町|村).*(長|知事|役所|役場|議会|委員会|局|公社|公団|"
    r"企業団|広域連合|一部事務組合)$"
)

_JICHITAI = None


def jichitai_mei(michi=None):
    """実在の都道府県・市区町村の名前。**手で並べない。**

    返せないときは**空の集合**。空なら「地方公共団体ではない」と読まれ、
    伏せる側に倒れる。
    """
    global _JICHITAI
    if _JICHITAI is not None and michi is None:
        return _JICHITAI
    de = set()
    if michi is None:
        michi = os.path.join(os.path.dirname(os.path.dirname(
            os.path.abspath(__file__))), "data", "ref", "jis-codes.json")
    try:
        with open(michi, encoding="utf-8") as f:
            for r in json.load(f):
                for k in ("pref", "city"):
                    v = (r.get(k) or "").strip()
                    if v:
                        de.add(v)
                p, c = (r.get("pref") or "").strip(), (r.get("city") or "").strip()
                if p and c:
                    de.add(p + c)
    except Exception:                                   # noqa: BLE001
        de = set()
    if michi is None:
        _JICHITAI = de
    _JICHITAI = de if _JICHITAI is None else _JICHITAI
    return de


def is_public(name):
    """国・地方公共団体か。**実在の名前と突き合わせる。**

    「◯村」で終わるだけでは決めない（**姓に多い**）。
    """
    t = (name or "").strip()
    if not t:
        return False
    if PUBLIC_RE.search(t):
        return True
    return t in jichitai_mei()

# 日本の戸籍の氏名はカタカナだけ・ローマ字だけにはならない。
# 「オークワ ほか」「F.O.B COOP」のような屋号を個人と取り違えないため。
_KANA_ONLY = re.compile(r"^[ァ-ヶー・\s]+$")
_HAS_LATIN = re.compile(r"[A-Za-zＡ-Ｚａ-ｚ]")

_BRACKETS = str.maketrans("（）［］〔〕　", "()[][] ")


def _norm(name):
    """全角と半角の括弧をそろえる。「(有）末広」のように混ざった実物がある。"""
    return (name or "").translate(_BRACKETS).strip()


# 法人格を表す語のうち、社名の「芯」ではない部分。前に付くか後ろに付くかは
# 会社ごとに違い、一覧によって書き方も変わる（㈱／(株)／株式会社）。
# NFKC を通すと ㈱ は (株) になる（㈲→(有)、㈳→(社)、㈶→(財)）。
# 丸囲みのまま比べると、同じ会社が別の会社に見える
_CORP_FORM = re.compile(
    r"(株式会社|有限会社|合同会社|合資会社|合名会社|相互会社|"
    r"一般社団法人|一般財団法人|公益社団法人|公益財団法人|"
    r"医療法人|学校法人|社会福祉法人|宗教法人|独立行政法人|"
    r"\(株\)|\(有\)|\(社\)|\(財\)|\(同\)|\(資\)|\(名\))")
# 共有者がいるときの「ほか1者」。**これは落とさない。**
# 付いたり消えたりするのは書き方の違いではなく、**共有者が変わったか、
# 書き落としたか**のどちらかで、どちらも中身の話（2026-09-19）。
#     尼崎都市開発株式会社  →  尼崎都市開発株式会社ほか6者  →  もとに戻る
# 落とすと、共同で持つ形に変わったことが見えなくなる。
# 全角と半角のゆれ（ほか1者／ほか１者）は NFKC がそろえる。
_ETC_TAIL = re.compile(r"\s*(ほか|他|外)\s*[0-9０-９]*\s*(名|者)?\s*$")


def corp_core(name):
    """【本番を毎回通る】直に呼ぶ所は無く、`same_corp()` の中から呼ばれる。
    `same_corp()` は `settisha.py` が小売業者の移り変わりを数えるときに呼ぶ。

    社名から、法人格の書き方と「ほか◯者」を落とした芯を返す。

    **同じ会社かどうかを比べるためだけに使う。画面には出さない。**
    芯を画面に出すと、こちらが会社名を書き換えたことになる（3.3）。

    実物にあった食い違い（2026-09-19）。どれも同じ会社。

        近鉄不動産(株)          近鉄不動産株式会社
        (株)山陽マルナカ         株式会社山陽マルナカ
        三菱UFJリース(株)       三菱ＵＦＪリース株式会社
        岩出建設(株)　ほか1者     岩出建設株式会社　ほか１者

    **「ほか◯者」は落とさない。** 同じ会社でも、共有者が付いたり消えたりするのは
    書き方ではなく中身の違い。
    """
    n = unicodedata.normalize("NFKC", _norm(name) or "")
    n = "".join(n.split())
    n = _CORP_FORM.sub("", n)
    return n.strip()


def co_owners(name):
    """共有者がいると書かれているか（「ほか」「ほか3者」など）。

    何者いるかまでは分からないことが多い（「ほか」だけの表記がある）。
    **いる／いないしか言わない。** 人数を数えたいなら書かれている数をそのまま出す。
    """
    n = unicodedata.normalize("NFKC", _norm(name) or "")
    return bool(_ETC_TAIL.search("".join(n.split())))


def same_corp(a, b):
    """2つの社名が、書き方の違いだけかどうか。

    **「違う」と出ても、同じ会社のことがある。** 商号変更・合併・持株会社化は
    名前そのものが変わるので、ここでは見分けられない。
        京阪電気鉄道株式会社 → 京阪ホールディングス株式会社（持株会社化）
        中央三井信託銀行㈱   → 三井住友信託銀行㈱（合併）
    見分けるには法人番号が要る。無いうちは「**表記が変わった**」までしか
    言えない（3.3「そのデータが何であるかを書く」）。
    """
    ca, cb = corp_core(a), corp_core(b)
    return bool(ca) and ca == cb


def is_corp(name):
    """法人格を表す語を含むか。含まなければ個人として扱う。"""
    n = _norm(name)
    if not n:
        return False
    if any(w in n for w in CORP_WORDS):
        return True
    # **`PUBLIC_RE` を直に見ない。** 実在の名前と突き合わせる `is_public` を通す
    # （2026-09-19 の監査 r1-39。「中村」が法人扱いになっていた）
    if is_public(n):
        return True
    core = re.sub(r"\s*(ほか|他)\s*[0-9０-９]*\s*者?\s*$", "", n).strip()
    if core and (_KANA_ONLY.match(core) or _HAS_LATIN.search(core)):
        return True
    return False


# ---------------------------------------------------------------- 名前の出し方

# 一次情報の側が、すでに伏せて公表しているときの書き方。
# これを個人名と間違えて二重に伏せないため、先に拾う。
ALREADY_HIDDEN = {"個人", "個人名", "非公開", "個人情報"}

# 名前ではないもの。「未定」「物品販売業を営む店舗」など、一次情報が
# 名前の代わりに書いている文言。個人と取り違えて「個人」に変えない。
_PLACEHOLDER = re.compile(
    r"^[（(\[]?\s*(未定|未詳|不明|なし|無し|―|—|ー|-|－|‐)\s*[0-9０-９]*\s*者?\s*[)）\]]?$"
    r"|^物品販売業"
)


def is_placeholder(name):
    """名前が入るべき場所に、名前以外の文言が入っているか。"""
    n = (name or "").strip()
    return bool(n) and bool(_PLACEHOLDER.match(n))


def redact_name(name, names=()):
    """画面に出す用。法人ならそのまま。個人なら "個人" を返す。

    names は、そのレコードに並ぶ他の名前（任意・渡さなくてよい）。
    **names は戻り値を1文字も変えない。安全性は1ミリも変わらない。**
    変わるのは、呼ぶ側が付ける名札だけ——つまり「この伏せ方でよかったのか」に
    人があとから気づけるかどうかだけ。伏せるか伏せないかを names に頼らせない。

    空欄にしない。空欄だと「取れなかった」のか「個人だから伏せた」のかが
    読者に分からない。伏せたことは、伏せたと書く（3.1）。
    """
    n = (name or "").strip()
    if not n:
        return ""
    if n in ALREADY_HIDDEN:
        return "個人"
    if is_placeholder(n):
        return n
    return n if is_corp(n) else "個人"


def party_for_index(name):
    """index.json の party に入れる用。法人ならそのまま、個人なら空文字。

    redact_name() とは別の関数にする。機械が読むデータに "個人" という
    文字列を入れると、全国の別人が同じ名前として扱われ、横断ハブで混ざる（3.1）。
    """
    n = (name or "").strip()
    if not n or n in ALREADY_HIDDEN or is_placeholder(n):
        return ""
    return n if is_corp(n) else ""


def party_kind(name, disclosed=True):
    """当事者の出し方。4つのどれか。3.1 の表と対応する。

    "corp"        法人と分かった
    "individual"  個人（法人と確かめられない場合を含む）
    "undisclosed" 一次情報の側が名前を載せていない（disclosed=False）
    "none"        そもそも当事者を持たない制度のレコード

    disclosed=False を渡してよいのは、一次情報に名前の欄が無いことを
    確かめたときだけ。欄はあるのに読めていないときは individual にする。
    こちらの解析の穴が、そのまま地番の公開になるため（3.1）。
    """
    if name is None:
        return "none"
    if not disclosed:
        return "undisclosed"
    n = (name or "").strip()
    if not n or is_placeholder(n):
        # 名前の欄はあるが、そこに名前が入っていない。相手が載せていないのか、
        # こちらが読めていないのかを決めつけない。安全な側に倒す（3.1）
        return "individual"
    if n in ALREADY_HIDDEN:
        return "individual"
    return "corp" if is_corp(n) else "individual"


# ------------------------------------------------ 名前の欄かどうか（列の見出しから）

# 表の列の見出しが「当事者の名前そのもの」を指しているか。
# 様式が変わって列の対応づけから漏れた名前の欄を受け止める網なので、
# 表記のゆれ（空白・括弧・中黒）で破れないようにする。
#
# 監査で、parse.py と merge.py が同じ判定を別々に持っていて、落とす記号が
# 違うために「設置者（氏名）」が片方だけすり抜けた。判定はここ1か所に置く。

_KEY_NOISE = re.compile(r"[\s　（）()・、．.：:／/]")

# 当事者を指す語
# **4サイトぶんを合わせて持つ**（正本5節「当事者を指す列名」）。
# 2026-09-20 の監査（r1-40）で、正本より6語少ないのが見つかった。
# **落ちている側の間違い**なので、静かに効く——落とした列は privacy を
# 通らないまま公開される。検査が正本と突き合わせる（手写しの一覧をやめない
# 代わりに、ずれたら鳴らす）
_PARTY_WORD = re.compile(
    r"(氏名|名義|代表者|代表取締役|届出者|申請者|設置者|小売業者|事業者|所有者|世帯主"
    r"|落札者|落札者名|契約相手方|契約の相手方|買受人|譲受人)")

# 当事者の語を含んでいても、名前の欄ではないもの。実物にある列名から。
# 「設置者対応」（設置者がどう応じたか）、「設置者意見」など、
# 語のうしろに別の意味の語が来る形。名前は列名の末尾に来る
_NOT_A_NAME_TAIL = re.compile(
    r"(対応|意見|理由|区分|種別|有無|概要|状況|内容|備考|日|日時|年月日|期間|"
    r"場所|番号|面積|台数|時間|品目|計画|届出|変更|移転|廃止)$")


def norm_key(name):
    """【本番を毎回通る】直に呼ぶ所は無く、`is_party_column()` の中から呼ばれる。
    `is_party_column()` は `parse.py` と `merge.py` が列の見出しを見るときに呼ぶ。

    列の見出しを突き合わせる形にそろえる。空白・括弧・中黒などを落とす。"""
    return _KEY_NOISE.sub("", str(name or ""))


def is_party_column(name):
    """列の見出しが、当事者の名前そのものを指しているか。

    「設置者」「小売業者」「代表者氏名」「設置者（氏名）」「小売業者 （名称）」は真。
    「設置者対応」「変更理由」「縦覧場所」は偽。

    真のときは、その列の値を当事者と同じ規則で扱う（法人はそのまま、個人は伏せる）。
    網なので、迷ったら真に倒したいところだが、偽に倒しても値が消えるだけで
    漏れはしない列（設置者対応）が実在するので、末尾の語で分ける。
    """
    k = norm_key(name)
    if not k or not _PARTY_WORD.search(k):
        return False
    return not _NOT_A_NAME_TAIL.search(k)


# ---------------------------------------------------------------- 所在地の粒度

# 「ほか」「外7筆」「他2筆」などの、地番のあとに付く書き足し
_TAIL = re.compile(r"\s*(ほか|外|他)\s*\d*\s*筆?\s*$")
# 「一丁目」「1丁目」「１丁目」まで
_CHOME = re.compile(r"^(.*?[0-9０-９一二三四五六七八九十]+\s*丁目)")
# 丁目とその直前の数字。丸めたあとに残ってよい数字はこれだけ
_CHOME_NUM = re.compile(r"[0-9０-９一二三四五六七八九十]+\s*丁目$")
_HAS_NUM = re.compile(r"[0-9０-９]")
# 丁目が無く、数字が3つ以上ハイフンでつながっているとき（梅田1-1-1 → 梅田1）。
# 数字が2つだけ（永代町329-1）は「番地-枝番」なので、この形にはしない
_HYPHEN3 = re.compile(
    r"^(.*?[0-9０-９]+)[-‐−－ー―][0-9０-９]+[-‐−－ー―][0-9０-９]")
# それも無いとき、最初の算用数字の手前まで（池田下町３７７番１ → 池田下町）
_FIRST_NUM = re.compile(r"^(.*?)(?=[0-9０-９])")


def redact_addr(addr, kind):
    """所在地の粒度。kind は party_kind() の戻り値。

    "individual" のときだけ町丁目まで丸める。
    corp / undisclosed / none は地番まで出す。
    """
    a = (addr or "").strip()
    if kind != "individual" or not a:
        return a

    a = _TAIL.sub("", a)

    m = _CHOME.match(a)
    if m:
        r = m.group(1).replace(" ", "")
        # 丸めたあとに残ってよい数字は「丁目」の直前の1つだけ。
        # ほかに数字が混じっていたら、住所が2つつながっているなどの
        # 取りこぼしなので、安全な側に倒して下の規則に落とす
        if not _HAS_NUM.search(_CHOME_NUM.sub("", r)):
            return r

    m = _HYPHEN3.match(a)
    if m and m.group(1).strip():
        # 「梅田1-1-1」の最初の数字は丁目。丁目と書いて返す。
        # そのまま「梅田1」にすると、もう一度この関数に通したときに
        # 「梅田」まで削れてしまう。同じ値を返し続ける形にしておく
        return _TAIL.sub("", m.group(1)).strip() + "丁目"

    m = _FIRST_NUM.match(a)
    if m and m.group(1).strip():
        return _TAIL.sub("", m.group(1)).strip().rstrip("　 ")

    return a


# ---------------------------------------------------------------- 小さい母数

# 伏せる上限。1件と2件は伏せる。0件と3件以上はそのまま出す（3.2）。
SUPPRESS_MAX = 2

# 率を出さない人口の下限（3.2）。
MIN_POPULATION = 500


def suppress_rate(count, population):
    """率を伏せるべきか。

    【本番から呼ばれていない】**この repo の本番から、という意味。**
    率を出していない（人口の分母を持たない）。署名だけそろえてある（5節の線3）。

    **`common/` は姉妹サイトがコピーして使うファイル**（4節）。消す相手ではない。
    2026-09-22 に数えたところ、**街頭窃盗統計の本番が呼んでいる**
    （`scripts/build.py` の3か所。`index.json` の `rate_suppressed` を作る）。
    **「この repo から呼ばれていない」を「どこからも使われていない」と読まない。**

    **呼ばれていないと書いておかないと、次に読む人が「ここを通っているから
    安全」と思い込む**（9節）。
人口500人未満、または件数が1件か2件で True（3.2）。

    **0件は伏せない。** 3.2 が心配しているのは「小さい母数で率が跳ね上がる」
    ことで、0件はその逆。件数そのものを 0 と出しているので、率を出しても
    戻るものが無い。0件を伏せると、本当に0件の升が「率を出していない」側に
    落ちて、3.2 の「伏せた升と本当に0件の升は見た目で分ける」と食い違う。

    **人口の小ささも同じ理由で効かない。** 9件÷400人の率に400を掛ければ
    9件が戻るが、0件には戻る先が無い。ここを「人口を先に見る」順で書いて
    いたため、suppress_rate(0, 400) が True を返していた。
    docstring は正しく、その下のコードが違っていた（2026-09-19、開発系が
    自分の実装で見つけ、突き合わせでこちらにも同じものが見つかった）。

    **ただし人口0だけは、0件でも True。** 伏せたいからではなく、
    **率そのものが定義できない**から（0では割れない）。理由が違うので、
    呼ぶ側はここを「伏せた」と書かない。「率が出せない」と書く（3.2）。
    0件を先に見る形に直したとき、街頭窃盗統計が人口0の町丁目54件で
    ゼロ除算した。**直しが新しい不具合を作る例**（2026-09-19）。
    """
    # 順は3段。入れ替えるとどれかが壊れる（2026-09-19）
    #   1. 人口0    率そのものが定義できない。0では割れない
    #   2. 0件      伏せない。戻る先が無い
    #   3. 小人口・1〜2件  伏せる
    if (population or 0) <= 0:
        return True
    if (count or 0) == 0:
        return False
    if (population or 0) < MIN_POPULATION:
        return True
    return 1 <= (count or 0) <= SUPPRESS_MAX


# 升の中身が「人ではない」と言える当事者の種類。
# `corp` は法人、`none` はそもそも当事者を持たない制度（6節の4値）。
# **`undisclosed` は入れない**——一次情報が名前を載せていないだけで、
# 個人かもしれない。「分からない」は伏せる側に倒す（9節）
HITO_DENAI = frozenset({"corp", "none"})


def hito_ga_iru(kinds):
    """その升に人が混ざっているか。**空は「いる」に倒す**（9節）。

    2026-09-20、中島さんの指摘——**「1〜2件の升は法人には当てはまらない。
    1件でも法人ならあり」。** 3.2 が守っているのは**人**なので、
    中身が法人だけの升には守るものが無い。
    """
    k = set(kinds or ())
    return (not k) or bool(k - HITO_DENAI)


def masked(n, kinds=None):
    """升の値そのもの。1件と2件は None（伏せた）、0件と3件以上はその数（3.2）。

    bucket_count() は人に見せる文字列、こちらは機械が持つ値。
    6節の count / count_label の2本立てと同じ分け方で、count 側がこれ。

    **出力に実数を書くときは必ずここを通す。** 画面が伏せ字にしていても、
    元のファイルに実数が入っていれば伏せたことにならない。

    この関数が無かったあいだ、3つのサイトが同じ判断をそれぞれ手で書いていた。
    たまたま3つとも合っていたが、次に書く人が外す（2026-09-17）。

    ## `kinds` を渡せるのは、**個票を全部出しているサイトだけ**

    渡すと、**中身が法人だけの升は実数のまま**出る（3.2 が守るのは人なので、
    人がいない升には守るものが無い）。

    **個票を出さないサイトでは渡してはいけない。** そこで選り分けると、
    **伏せ字そのものが「ここに人がいる」という印**になる。
    升しか出していないサイトでは、その印が唯一の手がかりになってしまう。
    個票を全部出しているサイトでは、個票の側に既に「個人」と書いてあるので、
    升から新しく分かることは何も増えない。

    既定は `None`＝**今までどおり全部伏せる**。渡さない側は何も変わらない。
    """
    n = int(n or 0)
    if 1 <= n <= SUPPRESS_MAX and hito_ga_iru(kinds):
        return None
    return n


def bucket_count(n, kinds=None):
    """件数の表示。1件と2件は "1-2"、それ以外は str(n)（3.2）。

    `kinds` の扱いは masked() と同じ。**2本立てで食い違わせない**
    （6節 count / count_label は必ず同じ判断から作る）。
    """
    n = int(n or 0)
    if n <= 0:
        return "0"
    if n <= SUPPRESS_MAX and hito_ga_iru(kinds):
        return f"1-{SUPPRESS_MAX}"
    return str(n)


# ---------------------------------------------------------------- 3.1 居住用

# 居住用の建物を表す語。**1つでも当たったら個票にしない。**
#
# **語の切れ目を書く。** 文字列を含むかだけで見ると、逆のものを拾う。
# 2026-09-19、実データでこの2件を誤って拾った——
#   「三井アウトレットパークマリンピア神**戸建**替計画」  ← 「戸建」
#   「ライフ**コーポ**レーション」                        ← 「コーポ」（法人の語）
# 「壊れたものを探す見張りが、正しいものを壊れていると言う」（9節）
_SUMAI = (
    "居宅", "住宅", "共同住宅", "集合住宅", "マンション", "アパート",
    "コーポ(?!レーション|レート)",      # コーポレーションは法人
    "ハイツ", "一戸建", "戸建て", "戸建住宅",   # 「神戸建替」を拾わない
    "長屋(?!町)", "寄宿舎", "下宿", "社宅", "官舎", "寮(?!歌)",
    "居室", "専有部分", "区分所有",
)
_SUMAI_RE = re.compile("|".join(_SUMAI))
# 住居系の用途地域（zoning.py が返す名前と同じ書き方）。
# **建物の語より先に見る。** 「住居」だけを建物の語に入れると
# 「第一種低層住居専用地域」が「居住用途の建物」という理由で落ちる。
# 結果は同じ（落とす）だが、**理由が違うと記録が嘘になる**
_SUMAI_YOUTO = ("住居専用地域", "住居地域", "田園住居")
# 入札・公売の途中。3.1「競売・公売の入札中物件」
_NYUUSATSU = ("入札中", "入札期間中", "公告中", "受付中", "開札前")


def residential_reason(*texts):
    """人が住んでいるかもしれないか。

    【本番から呼ばれていない】**この repo の本番から、という意味。**
    呼んでいないことを 2026-09-19 に測って決めた。届出の対象は 1,000㎡超の
    店舗で、当事者はほぼ法人。住居系用途地域の個票が 207 件、店名に住まいの語が
    入る複合ビルが 12 件あるが、**どれも「誰か一人の住まい」にはならない**
    （3.1 の表）。

    **`common/` は姉妹サイトがコピーして使うファイル**（4節）。消す相手ではない。
    2026-09-22 に数えたところ、**競売統計の本番が呼んでいる**
    （同じファイルの中で、別の関数が呼ぶ形）。
    競売・公売のように**売られる対象が住居そのもの**のサイトでは、必ず呼ぶこと。
    **「この repo から呼ばれていない」を「どこからも使われていない」と読まない。**
個票にしてよければ空文字、しないなら理由。

    渡すのは「その物件が何か」を表す文字列（地目・用途地域・建物概要・所在地など）。
    **並べる順は問わない。** 呼ぶ側が欄をどう並べるかに、判定を依存させない。

        residential_reason("宅地 居宅 木造・平家建")  → "居住用途の建物"
        residential_reason("倉庫")                     → ""

    **判定に迷ったら空文字を返さない。** 個票にしない側に倒す（3.1）。
    何も渡されなかったとき・全部空のときも、空文字を返さない。
    「何も書いていない」は「住んでいない」の証拠にならない。

    **落としすぎは測る。** 落とした数と理由を数えて、記録に出すのは呼ぶ側
    （9節「数えなかったものを、数えなかった理由で分ける」）。
    """
    joined = unicodedata.normalize("NFKC", " ".join(t or "" for t in texts))
    if not joined.strip():
        return "何が建っているか分からない"
    for w in _NYUUSATSU:
        if w in joined:
            return "入札の途中"
    for w in _SUMAI_YOUTO:
        if w in joined:
            return "住居系の用途地域"
    if _SUMAI_RE.search(joined):
        return "居住用途の建物"
    return ""
