...
ここに書かない**——この段が毎回その場で数えて記録に出す。出どころは国土交通省の位置参照情報。
`ref_youto.py` も用途地域を探しに同じ場所（`nlftp.mlit.go.jp/isj/`）を見に行っているが、
2026-09-22 の実測では、その取得段はデータ本体に届いていない（正本4節）。
「用途地域と町丁目一覧は、同じ1回の実行で取りに行ける」は、2026-09-19 時点の見込みだった。
"""
import collections
import json
import os
