...
def test_条件付きGETを変わっていないと書いていないか():
    """**304 は「相手がそう言った」。こちらは中身を持っていない**（9節）。
    ...
    if "こちらは中身を1バイトも持っていない" not in md:
        raise AssertionError("304 の読み方が記録に書かれていない（9節）")
...
def test_記録の在りかを_def_のときに固めていないか():
    ...
    検査は通り続けるが、**見ているものが違う。**
    「処理成功」と「観測成功」を同一視しない（9節）の、検査の側の形。
...
def test_正本_県公報を新しいpublicへ持ち込まない決まりが在るか():
    """**9節が自分の中で食い違っていた**（2026-09-22 に見つけた）。

    9節の作り直し手順①「入れないもの」に `data/koho` が挙がっておらず、
    同じ9節の別の行が「兵庫県公報の写しは**公開用に置く**」と書いていた。
    **手順どおりに作り直すと、その日に外したものを入れ直す。**

    捕まえるのは3つ。

    ① 「入れないもの」に `data/koho` がディレクトリごと挙がっている
    ② 新しい public へ持ち込まない、と書いてある
    ③ **旧判断が「失効」と分かる形で残っていて、生きた規則として立っていない**
       （消すのではなく残す。消すと、また同じことを思いつく人がいる。
       ただし引用（`>`）の外に立っていると、拾い直される）
    """
    seihon = os.path.join(HERE, "docs", "kyotsu-shiyo.md")
    if not os.path.exists(seihon):
        return
    ...
    # ① ②
    #    **失効の理由を「46種類が未測定だから」と読ませない。**
    #    46種類を数えたのは data/koho/text（大店立地法の抜粋）で、
    #    外した対象の data/koho/full（全文）ではない（統括 2026-09-22）
    for kotoba in ("`data/koho/**`（ディレクトリごと全部）",
                   "新しい public にも持ち込まない",
                   "【2026-09-22 失効】",
                   "測った対象と、外した対象は、同じではなかった",
                   # **動いてはいけない期間は、仕組みとして動かない**（統括 2026-09-22）。
                   # 「数時間なら cron に当たらないだろう」に頼る形に戻っていないか
                   "最初の push より前に、新 repo の Actions を無効にする",
                   # ④の実地で分かった4点（2026-09-22）。**直しただけでは戻せる**
                   "「一覧が返る＝動く」ではない",
                   "Pages の組み立ては Actions の仕組みに乗っている",
                   "Rename の場所は Danger Zone ではない",
                   "繋がらない ≠ 止まった"):
        if kotoba not in text:
            raise AssertionError(
                f"正本9節に「{kotoba}」が無い。"
                "**作り直しの手順が、外したものを入れ直す形に戻っている**")

    # ③ 旧判断は引用の中だけ。引用の外に立っていたら、それは生きた規則。
    #    **2つの語が同じ行に在ることを当てにしない。** 折り返しで別の行に来る
    #    （2026-09-22、最初に書いた形は `koho` が前の行にあって一度も当たらなかった）
    ikiteru = [ln.strip()[:40] for ln in text.split("\n")
               if "公開用に置く" in ln and not ln.lstrip().startswith(">")]
    if ikiteru:
        raise AssertionError(
            "「公開用に置く」が、引用の外に %d か所ある: %s。"
            "**失効した判断が、生きた規則として立っている**"
            % (len(ikiteru), ikiteru[:2]))

    # **間違っていた言い切りが、生きた記述として残っていないか。**
    #    引用として「旧版はこう書いていた」と残すのはよい（消すとまた思いつく）。
    #    太字の言い切りのまま在ったら、それは生きた規則。
    if "**Pages の配信は Actions とは別**" in text:
        raise AssertionError(
            "正本9節に「Pages の配信は Actions とは別」が言い切りのまま在る。"
            "**2026-09-22 の実測では、Actions を止めている間は Pages の build が"
            "1回も走らなかった**")

    # 役割が3つに分かれているか。**-raw と -archive を混ぜない**
    for kotoba in ("-archive", "履歴保全専用"):
        if kotoba not in text:
            raise AssertionError(
                f"正本9節に「{kotoba}」が無い。"
                "旧 public の退避先と、動いている金庫が、同じ名前になっている")


def test_正本_人が写すものをコピー枠に入れる決まりが在るか():
    """**入力はミスのもと**（運営者・2026-09-22）。

    10節には「貼る値は1つずつコードブロックに入れる」が既に在ったが、
    **報告文そのもの**と **GitHub の URL** が対象に入っていなかった。
    参謀は同じ日に2回、コピー枠を付け忘れて指摘された。
    **1回目を「今回の依頼」として受け取り、決まりに昇格させなかった。**

    **覚えておくつもりのものは落ちる。**だから決まりの側に書き、ここで見張る。
    """
    seihon = os.path.join(HERE, "docs", "kyotsu-shiyo.md")
    if not os.path.exists(seihon):
        return
    with open(seihon, encoding="utf-8") as f:
        text = f.read()
    for kotoba in ("人が写すものは、全部コピー枠に入れる",
                   "入力はミスのもと",
                   "統括への報告文は、本文ごと枠に入れる",
                   "枠に入れたものを、地の文でもう一度書かない",
                   # **閉じた一覧にしない**（運営者・2026-09-22）。
                   # 「この5つだけ」と読める形に戻っていないか
                   "入力すべきものは、例外なく枠に入れる",
                   "これで全部ではない。迷ったら枠に入れる",
                   # **押すものは打たせない**（運営者・2026-09-22）。
                   # URL を枠に入れる形に戻っていないか
                   "押すものを打ち込ませない",
                   "飛ばすのはリンク、打たせるのは枠"):
        if kotoba not in text:
...
def test_県公報_公開側に1件も残っていないか():
    ...
    2026-09-22 に外した（統括判断 2026-09-21・正本6776行）。
