...
def kind_narabi(kinds):
    """画面に出す種類の順。**一覧に無い種類を落とさない。**

    2026-09-20、開発ちゃんが自分の所で同じ形を見つけた——
    **報告の組み立てが系統名の一覧で回っていて、一覧に無いものは
    収集も保存もされるのに紙にだけ出ない。**

    ここも同じだった。`for k in KIND_ORDER` で並べていたので、
    **知らない種類は個票のページが作られるのに、入口のリンクが出ない。**
    作られたページに、どこからも行けない。

    **今日は正しく動いている**（いまある6種類は全部一覧に在る）。
    **足した日に黙る。** 監査の r1-26 と同じ族。

    一覧は**順番のため**に残す（勝手に並べると読みにくい）。
    知らないものは**後ろに足す。落とさない。**
    """
    aru = {k for k, n in (kinds or {}).items() if k and n}
    shitteru = [k for k in KIND_ORDER if k in aru]
    return shitteru + sorted(aru - set(KIND_ORDER))
...
# **空でよい欄には理由を書く。理由が書けないなら、それは穴**（競売統計、2026-09-19）。
#
# 向こうが「出す欄を1つずつ空にして走らせる」をやって3つ黙った。
# こちらで同じことをしたら、**25欄のうち17欄が黙った。**
# `site` を空にしても、`generated_at` を空にしても、升の `city` を空にしても、
# 検査は1本も鳴らなかった。**向きが逆だと捕まらない**——
# 「ある値が入っているか」を見る検査は在るが、
# 「**空のものが出ていないか**」を見る検査が無かった。
#
# ここに書いた欄だけが空になってよい。**書いていない欄が空なら止める。**
...
def detail_page(r, by_ref, src_meta):
    ...
    addr_note = ""
    if r.get("address_nakatta"):
        # **「伏せた」と書かない。** 取ってきた一覧に所在地の欄が無かっただけ。
        # 神戸市と兵庫県の縦覧がこれ（2026-09-20 に実物で確かめた）。
        # 読者にとっては「あるが見せない」と「探しても無い」は別のこと
        addr_note = ('<span class="note">'
                     '（取ってきた一覧に所在地が入っていませんでした）</span>')
    elif r.get("address_redacted"):
...
def area_page(area, rows):
    ...
    chips = "".join(f'<a href="{rel}k/{esc(k)}.html">{esc(k)}<b>{n_(kinds[k])}</b></a>' for k in kind_narabi(kinds))
...
<li><b>市区町村ごとの件数のうち、設置者に個人が含まれる1〜2件の小さい数字は「1-2」と伏せています。</b> 設置者が法人だけの件数は、1件でもそのまま出します（伏せる理由は人を守るためなので、法人だけの数字には守るものがありません）。なお、このサイトは届出を1件ずつすべて載せているので、<b>この伏せ字が個人の特定を防ぐわけではありません。</b>姉妹サイトと同じ形でデータを出すために、伏せ方をそろえているものです。</li>
...
def index_page(all_recs, today):
    ...
    # 2026-09-19 に print の名乗りだけ直して、**画面の見出しを直し忘れていた**
    # （監査 r1-54。競売統計が同じ日に「市区町村92」→「升92／市区町村46」と訂正した形）。
    # **同じ間違いを、同じ日に、片方だけ直していた。**
    # **数え方も、index.json を作る側と同じ道を通す。**
    # 1回目は `city_code_of(pref, city)` で引いて 81 になった（正しくは112）。
    # あちらは住所からも市区町村を起こしているので、**別の道は別の数を出す。**
    # 「通った検査と、通った道は別」（9節）の、数え方版
    def _koodo(r):
        machi = r.get("area") if r.get("area") not in (None, "", "所在地不明") else ""
        return addrlib.normalize(r.get("pref", ""), machi,
                                 r.get("address", ""))["city_code"]
    shikuchoson = len({c for c in (_koodo(r) for r in all_recs) if c})
    area_chips = "".join(f'<a href="a/{esc(slug(a))}.html">{esc(a)}<b>{n_(n)}</b></a>' for a, n in sorted(areas.items(), key=lambda x: -x[1]))
    kind_chips = "".join(f'<a href="k/{esc(k)}.html">{badge(k)} <b>{n_(kinds[k])}</b></a>' for k in kind_narabi(kinds))
...
def build_index(recs, today):
    ...
    cells = Counter()
    tousha = defaultdict(set)      # 升ごとの当事者の種類（3.2 の判断に渡す）
    label = {}
    for r in recs:
        ...
        if n["city_code"] and city and date_:
            ...
            cells[k] += 1
            # **その升に人が混ざっているか**を一緒に持つ（3.2・2026-09-20）。
            # 3.2 が守っているのは人なので、**中身が法人だけの升には守るものが無い。**
            # このサイトは個票を全部出しているので、升から新しく分かることは増えない
            # （個票の側に既に「個人」と書いてある）
            tousha[k].add(r.get("operator_kind") or "individual")
        else:
    ...
    for (code, city, kind, period), c in sorted(cells.items(), key=lambda x: (x[0][1], x[0][2], x[0][3]), reverse=False):
        counts.append({"city_code": code, "city": city, "kind": kind, "period": period,
                       "count": privacy.masked(c, tousha[(code, city, kind, period)]),
                       "count_label": privacy.bucket_count(
                           c, tousha[(code, city, kind, period)])})
