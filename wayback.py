...
def backfill(sid, urls, budget, lines, prefixes=()):
    ...
    for url in urls:
        try:
            ...
            # **一覧の取得も 429/503 を見る**（監査 r1-37・2026-09-20）。
            # 本体の取得（下）は見ているのに、ここだけ見ずに**次の URL へ進んで**いた。
            # 相手は同じ1台なので、一覧で断られた直後に次を叩けば押し込んでいる（3.4）
            if is_busy(e):
                save_ledger(sid, led)
                raise Busy(f"保存の一覧で HTTP {e.code}")
    ...
    for pre in prefixes:
        try:
            ...
            # 上と同じ。**一覧で断られたら、その回は止まる**（3.4）
            if is_busy(e):
                save_ledger(sid, led)
                raise Busy(f"前方一致の一覧で HTTP {e.code}")
