# Excelを取ってきた結果
**新しく取れた 2本 / すでに持っていた 355本 / 取れなかった 2本**

### 兵庫県 大規模小売店舗立地法 縦覧状況

- Excel/CSV/PDF のリンク: 41 本

### 神戸市 大規模小売店舗立地法 届出状況等

- Excel/CSV/PDF のリンク: 17 本
  - **todoke1154.pdf** 294,071バイト … 届出書（PDF：288KB）
    - シート「(PDF)」 57行  様式第２ | （ | 第 | ６ | 条 | 関 | 係 | ）
  - **todoke1153.pdf** 241,965バイト … 届出書（PDF：237KB）
    - シート「(PDF)」 35行  様 | 式第 | ２ | （第６条関係） | ※ | 受 | 理年 | 月

### 大阪府 大規模小売店舗立地法

- Excel/CSV のリンク: 17 本

### 大阪市 大規模小売店舗立地法 手続き・届出

- Excel/CSV のリンク: 3 本
  - 手持ちあり ju_20260818.xlsx — 取り直し HTTPError: HTTP Error 404: Not Found（手持ちを使う）
  - 手持ちあり to20260630.xls — 取り直し HTTPError: HTTP Error 404: Not Found（手持ちを使う）
  - 手持ちあり si20260630.xls — 取り直し HTTPError: HTTP Error 404: Not Found（手持ちを使う）

### 和泉市 届出状況

- Excel/CSV/PDF のリンク: 58 本
  - 取れなかった Ion.pdf — HTTPError: HTTP Error 404: Not Found
  - 取れなかった marunaka-izumi.pdf — HTTPError: HTTP Error 404: Not Found

### 大阪狭山市 大規模小売店舗立地法

- Excel/CSV のリンク: 0 本

### 河内長野市 大規模小売店舗立地法のお知らせ

- Excel/CSV/PDF のリンク: 1 本

### （参考）大阪市オープンデータポータル 添付ファイル一覧

- まだページを保存していない

### （参考）大阪市オープンデータポータル（www側の入口）

- Excel/CSV のリンク: 0 本

### （参考）神戸市 利用規約・リンク・免責事項など

- Excel/CSV/PDF のリンク: 1 本

### （参考）兵庫県公報 検索用目録（Excel）

- Excel/CSV のリンク: 20 本

### 兵庫県 まちづくり審議会 大規模小売店舗等立地部会（議案）

- Excel/CSV/PDF のリンク: 191 本

### （参考）兵庫県 まちづくり審議会（調査審議の結果）

- Excel/CSV/PDF のリンク: 10 本

### （参考）経済産業省 大店立地法の届出状況について

- まだページを保存していない
