...
# 「取ってはいけない」のまま動かない。下の表は**運営会社の側の話**で、
# **施設の欄に流し込まない。**
UNEI_YAKUSOKU = (
...
KOUHO = (
    {"id": "gardens",   "mei": "阪急西宮ガーデンズ",   "basho": "兵庫県西宮市",
     ...
     "yakusoku": {"mita_hi": "2026-09-21", "kekka": "取ってよい",
                  # **2026-09-21、統括が「規約拒否」を撤回した。** 条件付きで戻した
                  "jouken": "**ページ・写真・説明文等を再公開しない。**"
                            "**公開ショップ情報から確認した事実の観測に限定する。**"
                            "（統括・2026-09-21）",
                  "riyuu": "**［相手が書いていること］** サイトポリシーの"
                           ...
                           "分ける文言も無い（明示なし）。**"
                           "**［以前のこちらの読みは取り下げた］** "
                           "「取って保存することが複製、抽出して配ることが二次利用」"
                           "と読んでいたが、**相手はそこまで書いていない**（ルール⑥）。"
                           "**［2026-09-21・統括の判断］** 従来の「規約拒否」を撤回し、"
                           "**上の条件のもとで観測候補に戻す。**"
                           "**条件が守れなくなったら、ここに戻って書き直す。**",
